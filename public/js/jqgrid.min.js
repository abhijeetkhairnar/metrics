...
index:S})});F.sort(function(S,aa){S=S.vSort;aa=aa.vSort;return f._compare(S,aa,J)});x=0;for(var C=q.length;x<C;){s=F[x].index;A.push(q[s]);x++}return A};this._getGroup=function(q,r,s,x,y){var A=[],F=null,J=null,B;a.each(f._getOrder(q,r,s,x,y),function(O,C){B=a.jgrid.getAccessor(C,r);if(B===undefined)B="";if(!f._equals(J,B)){J=B;F!==null&&A.push(F);F=f._group(r,B)}F.items.push(C)});F!==null&&A.push(F);return A};this.ignoreCase=function(){h=false;return f};this.useCase=function(){h=true;return f};this.trim=
...
this.id+"'></div>").append(k).insertBefore(p);a("<div class='loading ui-state-default ui-state-active' id='load_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(p);a(this).attr({cellspacing:"0",cellpadding:"0",border:"0",role:"grid","aria-multiselectable":!!this.p.multiselect,"aria-labelledby":"gbox_"+this.id});var m=function(u,t){u=parseInt(u,10);return isNaN(u)?t?t:0:u},v=function(u,t,w,z,D,R){var H=b.p.colModel[u],V=H.align,K='style="',P=H.classes,I=H.name,N=[];if(V)K+="text-align:"+V+";";
if(H.hidden===true)K+="display:none;";if(t===0)K+="width: "+g.headers[u].width+"px;";else if(H.cellattr&&a.isFunction(H.cellattr))if((u=H.cellattr.call(b,D,w,z,H,R))&&typeof u==="string"){u=u.replace(/style/i,"style").replace(/title/i,"title");if(u.indexOf("title")>-1)H.title=false;if(u.indexOf("class")>-1)P=undefined;N=u.split("style");if(N.length===2){N[1]=a.trim(N[1].replace("=",""));if(N[1].indexOf("'")===0||N[1].indexOf('"')===0)N[1]=N[1].substring(1);K+=N[1].replace(/'/gi,'"')}else K+='"'}if(!N.length){N[0]=
"";K+='"'}K+=(P!==undefined?' class="'+P+'"':"")+(H.title&&w?' title="'+a.jgrid.stripHtml(w)+'"':"");K+=' aria-describedby="'+b.p.id+"_"+I+'"';return K+N[0]},q=function(u){return u===undefined||u===null||u===""?"&#160;":b.p.autoencode?a.jgrid.htmlEncode(u):u+""},r=function(u,t,w,z,D){var R=b.p.colModel[w];if(typeof R.formatter!=="undefined"){u={rowId:u,colModel:R,gid:b.p.id,pos:w};t=a.isFunction(R.formatter)?R.formatter.call(b,t,u,z,D):a.fmatter?a.fn.fmatter(R.formatter,t,u,z,D):q(t)}else t=q(t);
...
t);b.p._index[z]=w}},O=function(u,t,w,z,D){var R=new Date,H=b.p.datatype!="local"&&b.p.loadonce||b.p.datatype=="xmlstring",V=b.p.datatype=="local"?"local":"xml";if(H){b.p.data=[];b.p._index={};b.p.localReader.id="_id_"}b.p.reccount=0;if(a.isXMLDoc(u)){if(b.p.treeANode===-1&&!b.p.scroll){J(t,false,true);w=1}else w=w>1?w:1;var K,P,I=0,N,X=0,la=0,fa=0,da,ga=[],qa,ba={},M,T,$=[],Ba=b.p.altRows===true?" "+b.p.altclass:"";b.p.xmlReader.repeatitems||(ga=A(V));da=b.p.keyIndex===false?b.p.xmlReader.id:b.p.keyIndex;
...
true)Fa=' style="display:none;"';for(;ra<pa;){M=u[ra];T=V(M,Ga+ra);K=w===0?0:w+1;K=(K+ra)%2==1?Ba:"";$.push("<tr"+Fa+' id="'+T+'" tabindex="-1" role="row" class ="ui-widget-content jqgrow ui-row-'+b.p.direction+""+K+'">');if(b.p.rownumbers===true){$.push(y(0,ra,b.p.page,b.p.rowNum));fa=1}if(b.p.multiselect===true){$.push(x(T,fa,ra));X=1}if(b.p.subGrid===true){$.push(a(b).jqGrid("addSubGridCell",X+fa,ra+w));la=1}if(b.p.xmlReader.repeatitems){qa||(qa=F(X+la+fa));var Ja=a(b.p.xmlReader.cell,M);a.each(qa,
function(ya){var ua=Ja[this];if(!ua)return false;N=ua.textContent||ua.text;ba[b.p.colModel[ya+X+la+fa].name]=N;$.push(s(T,N,ya+X+la+fa,ra+w,M))})}else for(K=0;K<ga.length;K++){N=a(ga[K],M).text();ba[b.p.colModel[K+X+la+fa].name]=N;$.push(s(T,N,K+X+la+fa,ra+w,M))}$.push("</tr>");if(b.p.grouping){K=b.p.groupingView.groupField.length;for(var Ha=[],Ia=0;Ia<K;Ia++)Ha.push(ba[b.p.groupingView.groupField[Ia]]);Ca=a(b).jqGrid("groupingPrepare",$,Ha,Ca,ba);$=[]}if(H||b.p.treeGrid===true){ba._id_=T;b.p.data.push(ba);
b.p._index[T]=b.p.data.length-1}if(b.p.gridview===false){a("tbody:first",t).append($.join(""));D&&b.p.afterInsertRow.call(b,T,ba,M);$=[]}ba={};I++;ra++;if(I==za)break}}if(b.p.gridview===true){P=b.p.treeANode>-1?b.p.treeANode:0;if(b.p.grouping){a(b).jqGrid("groupingRender",Ca,b.p.colModel.length);Ca=null}else b.p.treeGrid===true&&P>0?a(b.rows[P]).after($.join("")):a("tbody:first",t).append($.join(""))}if(b.p.subGrid===true)try{a(b).jqGrid("addSubGrid",X+fa)}catch(La){}b.p.totaltime=new Date-R;if(I>
0)if(b.p.records===0)b.p.records=pa;$=null;if(b.p.treeGrid===true)try{a(b).jqGrid("setTreeNode",P+1,I+P+1)}catch(Ma){}if(!b.p.treeGrid&&!b.p.scroll)b.grid.bDiv.scrollTop=0;b.p.reccount=I;b.p.treeANode=-1;b.p.userDataOnFooter&&a(b).jqGrid("footerData","set",b.p.userData,true);if(H){b.p.records=pa;b.p.lastpage=Math.ceil(pa/za)}z||b.updatepager(false,true);if(H)for(;I<pa;){M=u[I];T=V(M,I);if(b.p.xmlReader.repeatitems){qa||(qa=F(X+la+fa));var Ka=a(b.p.xmlReader.cell,M);a.each(qa,function(ya){var ua=Ka[this];
if(!ua)return false;N=ua.textContent||ua.text;ba[b.p.colModel[ya+X+la+fa].name]=N})}else for(K=0;K<ga.length;K++){N=a(ga[K],M).text();ba[b.p.colModel[K+X+la+fa].name]=N}ba._id_=T;b.p.data.push(ba);b.p._index[T]=b.p.data.length-1;ba={};I++}}},C=function(u,t,w,z,D){var R=new Date;if(u){if(b.p.treeANode===-1&&!b.p.scroll){J(t,false,true);w=1}else w=w>1?w:1;var H,V=b.p.datatype!="local"&&b.p.loadonce||b.p.datatype=="jsonstring";if(V){b.p.data=[];b.p._index={};b.p.localReader.id="_id_"}b.p.reccount=0;
if(b.p.datatype=="local"){t=b.p.localReader;H="local"}else{t=b.p.jsonReader;H="json"}var K=0,P,I,N=[],X,la=0,fa=0,da=0,ga,qa,ba={},M,T,$=[],Ba=b.p.altRows===true?" "+b.p.altclass:"";b.p.page=a.jgrid.getAccessor(u,t.page)||0;ga=a.jgrid.getAccessor(u,t.total);b.p.lastpage=ga===undefined?1:ga;b.p.records=a.jgrid.getAccessor(u,t.records)||0;b.p.userData=a.jgrid.getAccessor(u,t.userdata)||{};t.repeatitems||(X=N=A(H));H=b.p.keyIndex===false?t.id:b.p.keyIndex;if(N.length>0&&!isNaN(H)){if(b.p.remapColumns&&
b.p.remapColumns.length)H=a.inArray(H,b.p.remapColumns);H=N[H]}(qa=a.jgrid.getAccessor(u,t.root))||(qa=[]);ga=qa.length;u=0;var pa=parseInt(b.p.rowNum,10),ra=b.p.scroll?a.jgrid.randId():1;if(D)pa*=D+1;var Ca=a.isFunction(b.p.afterInsertRow),za={},Ga="";if(b.p.grouping&&b.p.groupingView.groupCollapse===true)Ga=' style="display:none;"';for(;u<ga;){D=qa[u];T=a.jgrid.getAccessor(D,H);if(T===undefined){T=ra+u;if(N.length===0)if(t.cell)T=a.jgrid.getAccessor(D,t.cell)[H]||T}P=w===1?0:w;P=(P+u)%2==1?Ba:"";
$.push("<tr"+Ga+' id="'+T+'" tabindex="-1" role="row" class= "ui-widget-content jqgrow ui-row-'+b.p.direction+""+P+'">');if(b.p.rownumbers===true){$.push(y(0,u,b.p.page,b.p.rowNum));da=1}if(b.p.multiselect){$.push(x(T,da,u));la=1}if(b.p.subGrid){$.push(a(b).jqGrid("addSubGridCell",la+da,u+w));fa=1}if(t.repeatitems){if(t.cell)D=a.jgrid.getAccessor(D,t.cell);X||(X=F(la+fa+da))}for(I=0;I<X.length;I++){P=a.jgrid.getAccessor(D,X[I]);$.push(s(T,P,I+la+fa+da,u+w,D));ba[b.p.colModel[I+la+fa+da].name]=P}$.push("</tr>");
if(b.p.grouping){P=b.p.groupingView.groupField.length;I=[];for(var Fa=0;Fa<P;Fa++)I.push(ba[b.p.groupingView.groupField[Fa]]);za=a(b).jqGrid("groupingPrepare",$,I,za,ba);$=[]}if(V||b.p.treeGrid===true){ba._id_=T;b.p.data.push(ba);b.p._index[T]=b.p.data.length-1}if(b.p.gridview===false){a("#"+a.jgrid.jqID(b.p.id)+" tbody:first").append($.join(""));Ca&&b.p.afterInsertRow.call(b,T,ba,D);$=[]}ba={};K++;u++;if(K==pa)break}if(b.p.gridview===true){M=b.p.treeANode>-1?b.p.treeANode:0;if(b.p.grouping)a(b).jqGrid("groupingRender",
za,b.p.colModel.length);else b.p.treeGrid===true&&M>0?a(b.rows[M]).after($.join("")):a("#"+a.jgrid.jqID(b.p.id)+" tbody:first").append($.join(""))}if(b.p.subGrid===true)try{a(b).jqGrid("addSubGrid",la+da)}catch(Ja){}b.p.totaltime=new Date-R;if(K>0)if(b.p.records===0)b.p.records=ga;if(b.p.treeGrid===true)try{a(b).jqGrid("setTreeNode",M+1,K+M+1)}catch(Ha){}if(!b.p.treeGrid&&!b.p.scroll)b.grid.bDiv.scrollTop=0;b.p.reccount=K;b.p.treeANode=-1;b.p.userDataOnFooter&&a(b).jqGrid("footerData","set",b.p.userData,
true);if(V){b.p.records=ga;b.p.lastpage=Math.ceil(ga/pa)}z||b.updatepager(false,true);if(V)for(;K<ga&&qa[K];){D=qa[K];T=a.jgrid.getAccessor(D,H);if(T===undefined){T=ra+K;if(N.length===0)if(t.cell)T=a.jgrid.getAccessor(D,t.cell)[H]||T}if(D){if(t.repeatitems){if(t.cell)D=a.jgrid.getAccessor(D,t.cell);X||(X=F(la+fa+da))}for(I=0;I<X.length;I++){P=a.jgrid.getAccessor(D,X[I]);ba[b.p.colModel[I+la+fa+da].name]=P}ba._id_=T;b.p.data.push(ba);b.p._index[T]=b.p.data.length-1;ba={}}K++}}},S=function(){function u(M){var T=
0,$,Ba,pa;if(M.groups!==undefined)for($=0;$<M.groups.length;$++){try{u(M.groups[$])}catch(ra){alert(ra)}T++}if(M.rules!==undefined){if(T>0){var Ca=N.select();N=a.jgrid.from(Ca)}try{for($=0;$<M.rules.length;$++){pa=M.rules[$];Ba=M.groupOp.toString().toUpperCase();if(I[pa.op]&&pa.field){if(T>0&&Ba&&Ba==="OR")N=N.or();N=I[pa.op](N,Ba)(pa.field,pa.data,z[pa.field])}T++}}catch(za){alert(za)}}}var t,w=false,z={},D=[],R=[],H,V,K;if(a.isArray(b.p.data)){var P=b.p.grouping?b.p.groupingView:false;a.each(b.p.colModel,
function(){V=this.sorttype||"text";if(V=="date"||V=="datetime"){if(this.formatter&&typeof this.formatter==="string"&&this.formatter=="date"){H=this.formatoptions&&this.formatoptions.srcformat?this.formatoptions.srcformat:a.jgrid.formatter.date.srcformat;K=this.formatoptions&&this.formatoptions.newformat?this.formatoptions.newformat:a.jgrid.formatter.date.newformat}else H=K=this.datefmt||"Y-m-d";z[this.name]={stype:V,srcfmt:H,newfmt:K}}else z[this.name]={stype:V,srcfmt:"",newfmt:""};if(b.p.grouping&&
this.name==P.groupField[0]){var M=this.name;if(typeof this.index!="undefined")M=this.index;D[0]=z[M];R.push(M)}if(!w&&(this.index==b.p.sortname||this.name==b.p.sortname)){t=this.name;w=true}});if(b.p.treeGrid)a(b).jqGrid("SortTree",t,b.p.sortorder,z[t].stype,z[t].srcfmt);else{var I={eq:function(M){return M.equals},ne:function(M){return M.notEquals},lt:function(M){return M.less},le:function(M){return M.lessOrEquals},gt:function(M){return M.greater},ge:function(M){return M.greaterOrEquals},cn:function(M){return M.contains},
nc:function(M,T){return T==="OR"?M.orNot().contains:M.andNot().contains},bw:function(M){return M.startsWith},bn:function(M,T){return T==="OR"?M.orNot().startsWith:M.andNot().startsWith},en:function(M,T){return T==="OR"?M.orNot().endsWith:M.andNot().endsWith},ew:function(M){return M.endsWith},ni:function(M,T){return T==="OR"?M.orNot().equals:M.andNot().equals},"in":function(M){return M.equals},nu:function(M){return M.isNull},nn:function(M,T){return T==="OR"?M.orNot().isNull:M.andNot().isNull}},N=a.jgrid.from(b.p.data);
if(b.p.ignoreCase)N=N.ignoreCase();if(b.p.search===true){var X=b.p.postData.filters;if(X){if(typeof X=="string")X=a.jgrid.parse(X);u(X)}else try{N=I[b.p.postData.searchOper](N)(b.p.postData.searchField,b.p.postData.searchString,z[b.p.postData.searchField])}catch(la){}}if(b.p.grouping){N.orderBy(R,P.groupOrder[0],D[0].stype,D[0].srcfmt);P.groupDataSorted=true}if(t&&b.p.sortorder&&w)b.p.sortorder.toUpperCase()=="DESC"?N.orderBy(b.p.sortname,"d",z[t].stype,z[t].srcfmt):N.orderBy(b.p.sortname,"a",z[t].stype,
z[t].srcfmt);X=N.select();var fa=parseInt(b.p.rowNum,10),da=X.length,ga=parseInt(b.p.page,10),qa=Math.ceil(da/fa),ba={};X=X.slice((ga-1)*fa,ga*fa);z=N=null;ba[b.p.localReader.total]=qa;ba[b.p.localReader.page]=ga;ba[b.p.localReader.records]=da;ba[b.p.localReader.root]=X;X=null;return ba}}},aa=function(){b.grid.hDiv.loading=true;if(!b.p.hiddengrid)switch(b.p.loadui){case "enable":a("#load_"+a.jgrid.jqID(b.p.id)).show();break;case "block":a("#lui_"+a.jgrid.jqID(b.p.id)).show();a("#load_"+a.jgrid.jqID(b.p.id)).show()}},
...
b.p.groupingView.groupField[0]+" "+b.p.groupingView.groupOrder[0]+", "+w[D.sort]}a.extend(b.p.postData,w);var K=!b.p.scroll?1:b.rows.length-1;if(a.isFunction(b.p.datatype))b.p.datatype.call(b,b.p.postData,"load_"+b.p.id);else{a.isFunction(b.p.beforeRequest)&&b.p.beforeRequest.call(b);z=b.p.datatype.toLowerCase();switch(z){case "json":case "jsonp":case "xml":case "script":a.ajax(a.extend({url:b.p.url,type:b.p.mtype,dataType:z,data:a.isFunction(b.p.serializeGridData)?b.p.serializeGridData.call(b,b.p.postData):
b.p.postData,success:function(P){z==="xml"?O(P,b.grid.bDiv,K,u>1,V):C(P,b.grid.bDiv,K,u>1,V);R&&R.call(b,P);t&&b.grid.populateVisible();if(b.p.loadonce||b.p.treeGrid)b.p.datatype="local";sa()},error:function(P,I,N){a.isFunction(b.p.loadError)&&b.p.loadError.call(b,P,I,N);sa()},beforeSend:function(P){aa();a.isFunction(b.p.loadBeforeSend)&&b.p.loadBeforeSend.call(b,P)}},a.jgrid.ajaxOptions,b.p.ajaxGridOptions));break;case "xmlstring":aa();w=a.jgrid.stringToDoc(b.p.datastr);O(w,b.grid.bDiv);H&&b.p.loadComplete.call(b,
w);b.p.datatype="local";b.p.datastr=null;sa();break;case "jsonstring":aa();w=typeof b.p.datastr=="string"?a.jgrid.parse(b.p.datastr):b.p.datastr;C(w,b.grid.bDiv);H&&b.p.loadComplete.call(b,w);b.p.datatype="local";b.p.datastr=null;sa();break;case "local":case "clientside":aa();b.p.datatype="local";w=S();C(w,b.grid.bDiv,K,u>1,V);R&&R.call(b,w);t&&b.grid.populateVisible();sa()}}}};k=function(u,t){var w="",z="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>",
D="",R,H,V,K,P=function(I){var N;if(a.isFunction(b.p.onPaging))N=b.p.onPaging.call(b,I);b.p.selrow=null;if(b.p.multiselect){b.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(b.p.id),b.grid.hDiv).attr("checked",false)}b.p.savedRow=[];if(N=="stop")return false;return true};u=u.substr(1);t+="_"+u;R="pg_"+u;H=u+"_left";V=u+"_center";K=u+"_right";a("#"+a.jgrid.jqID(u)).append("<div id='"+R+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='"+
H+"' align='left'></td><td id='"+V+"' align='center' style='white-space:pre;'></td><td id='"+K+"' align='right'></td></tr></tbody></table></div>").attr("dir","ltr");if(b.p.rowList.length>0){D="<td dir='"+j+"'>";D+="<select class='ui-pg-selbox' role='listbox'>";for(H=0;H<b.p.rowList.length;H++)D+='<option role="option" value="'+b.p.rowList[H]+'"'+(b.p.rowNum==b.p.rowList[H]?' selected="selected"':"")+">"+b.p.rowList[H]+"</option>";D+="</select></td>"}if(j=="rtl")z+=D;if(b.p.pginput===true)w="<td dir='"+
...
"default";else{a(this).addClass("ui-state-hover");this.style.cursor="pointer"}},function(){if(!a(this).hasClass("ui-state-disabled")){a(this).removeClass("ui-state-hover");this.style.cursor="default"}});a("#first"+a.jgrid.jqID(t)+", #prev"+a.jgrid.jqID(t)+", #next"+a.jgrid.jqID(t)+", #last"+a.jgrid.jqID(t)).click(function(){var I=m(b.p.page,1),N=m(b.p.lastpage,1),X=false,la=true,fa=true,da=true,ga=true;if(N===0||N===1)ga=da=fa=la=false;else if(N>1&&I>=1)if(I===1)fa=la=false;else{if(!(I>1&&I<N))if(I===
N)ga=da=false}else if(N>1&&I===0){ga=da=false;I=N-1}if(this.id==="first"+t&&la){b.p.page=1;X=true}if(this.id==="prev"+t&&fa){b.p.page=I-1;X=true}if(this.id==="next"+t&&da){b.p.page=I+1;X=true}if(this.id==="last"+t&&ga){b.p.page=N;X=true}if(X){if(!P(this.id))return false;ja()}return false})}b.p.pginput===true&&a("input.ui-pg-input","#"+R).keypress(function(I){if((I.charCode?I.charCode:I.keyCode?I.keyCode:0)==13){b.p.page=a(this).val()>0?a(this).val():b.p.page;if(!P("user"))return false;ja();return false}return this})};
...
false;b.p.rowList=[]}b.p.data.length&&B();var na="<thead><tr class='ui-jqgrid-labels' role='rowheader'>",Da,va,E,G,U,L,Q,W;va=W="";if(b.p.shrinkToFit===true&&b.p.forceFit===true)for(h=b.p.colModel.length-1;h>=0;h--)if(!b.p.colModel[h].hidden){b.p.colModel[h].resizable=false;break}if(b.p.viewsortcols[1]=="horizontal"){W=" ui-i-asc";va=" ui-i-desc"}Da=n?"class='ui-th-div-ie'":"";W="<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+W+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+
j+"'></span>";W+="<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+va+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+j+"'></span></span>";for(h=0;h<this.p.colNames.length;h++){va=b.p.headertitles?' title="'+a.jgrid.stripHtml(b.p.colNames[h])+'"':"";na+="<th id='"+b.p.id+"_"+b.p.colModel[h].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+j+"'"+va+">";va=b.p.colModel[h].index||b.p.colModel[h].name;na+="<div id='jqgh_"+b.p.id+"_"+b.p.colModel[h].name+"' "+
Da+">"+b.p.colNames[h];b.p.colModel[h].width=b.p.colModel[h].width?parseInt(b.p.colModel[h].width,10):150;if(typeof b.p.colModel[h].title!=="boolean")b.p.colModel[h].title=true;if(va==b.p.sortname)b.p.lastsort=h;na+=W+"</div></th>"}na+="</tr></thead>";W=null;a(this).append(na);a("thead tr:first th",this).hover(function(){a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")});if(this.p.multiselect){var Y=[],ha;a("#cb_"+a.jgrid.jqID(b.p.id),this).bind("click",function(){b.p.selarrrow=
...
function(u){Q=a(u.target).closest("tr.jqgrow");a(Q).removeClass("ui-state-hover");return false});var ca,ta;a(b).before(g.hDiv).click(function(u){L=u.target;Q=a(L,b.rows).closest("tr.jqgrow");if(a(Q).length===0||Q[0].className.indexOf("ui-state-disabled")>-1)return this;var t=a(L).hasClass("cbox"),w=true;if(a.isFunction(b.p.beforeSelectRow))w=b.p.beforeSelectRow.call(b,Q[0].id,u);if(L.tagName=="A"||(L.tagName=="INPUT"||L.tagName=="TEXTAREA"||L.tagName=="OPTION"||L.tagName=="SELECT")&&!t)return this;
if(w===true){if(b.p.cellEdit===true)if(b.p.multiselect&&t)a(b).jqGrid("setSelection",Q[0].id,true);else{ca=Q[0].rowIndex;ta=a.jgrid.getCellIndex(L);try{a(b).jqGrid("editCell",ca,ta,true)}catch(z){}}else if(b.p.multikey)if(u[b.p.multikey])a(b).jqGrid("setSelection",Q[0].id,true);else{if(b.p.multiselect&&t){t=a("#jqg_"+a.jgrid.jqID(b.p.id)+"_"+Q[0].id).attr("checked");a("#jqg_"+a.jgrid.jqID(b.p.id)+"_"+Q[0].id).attr("checked",!t)}}else{if(b.p.multiselect&&b.p.multiboxonly)if(!t){a(b.p.selarrrow).each(function(D,
R){var H=b.rows.namedItem(R);a(H).removeClass("ui-state-highlight");a("#jqg_"+a.jgrid.jqID(b.p.id)+"_"+a.jgrid.jqID(R)).attr("checked",false)});b.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(b.p.id),b.grid.hDiv).attr("checked",false)}a(b).jqGrid("setSelection",Q[0].id,true)}if(a.isFunction(b.p.onCellSelect)){ca=Q[0].id;ta=a.jgrid.getCellIndex(L);b.p.onCellSelect.call(b,ca,ta,a(L).html(),u)}u.stopPropagation()}else return this}).bind("reloadGrid",function(u,t){if(b.p.treeGrid===true)b.p.datatype=b.p.treedatatype;
t&&t.current&&b.grid.selectionPreserver(b);if(b.p.datatype=="local"){a(b).jqGrid("resetSelection");b.p.data.length&&B()}else if(!b.p.treeGrid){b.p.selrow=null;if(b.p.multiselect){b.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(b.p.id),b.grid.hDiv).attr("checked",false)}b.p.savedRow=[]}b.p.scroll&&J(b.grid.bDiv,true,false);if(t&&t.page){var w=t.page;if(w>b.p.lastpage)w=b.p.lastpage;if(w<1)w=1;b.p.page=w;b.grid.bDiv.scrollTop=b.grid.prevRowHeight?(w-1)*b.grid.prevRowHeight*b.p.rowNum:0}if(b.grid.prevRowHeight&&
b.p.scroll){delete b.p.lastpage;b.grid.populateVisible()}else b.grid.populate();return false});a.isFunction(this.p.ondblClickRow)&&a(this).dblclick(function(u){L=u.target;Q=a(L,b.rows).closest("tr.jqgrow");if(a(Q).length===0)return false;ca=Q[0].rowIndex;ta=a.jgrid.getCellIndex(L);b.p.ondblClickRow.call(b,a(Q).attr("id"),ca,ta,u);return false});a.isFunction(this.p.onRightClickRow)&&a(this).bind("contextmenu",function(u){L=u.target;Q=a(L,b.rows).closest("tr.jqgrow");if(a(Q).length===0)return false;
b.p.multiselect||a(b).jqGrid("setSelection",Q[0].id,true);ca=Q[0].rowIndex;ta=a.jgrid.getCellIndex(L);b.p.onRightClickRow.call(b,a(Q).attr("id"),ca,ta,u);return false});g.bDiv=document.createElement("div");if(n)if(String(b.p.height).toLowerCase()==="auto")b.p.height="100%";a(g.bDiv).append(a('<div style="position:relative;'+(n&&a.browser.version<8?"height:0.01%;":"")+'"></div>').append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({height:b.p.height+(isNaN(b.p.height)?"":"px"),width:g.width+
...
a(document).mouseup(function(){if(g.resizing){g.dragEnd();return false}return true});b.formatCol=v;b.sortData=ia;b.updatepager=function(u,t){var w,z,D,R,H,V,K,P="",I=b.p.pager?"_"+a.jgrid.jqID(b.p.pager.substr(1)):"";D=parseInt(b.p.page,10)-1;if(D<0)D=0;D*=parseInt(b.p.rowNum,10);H=D+b.p.reccount;if(b.p.scroll){w=a("tbody:first > tr:gt(0)",b.grid.bDiv);D=H-w.length;b.p.reccount=w.length;if(z=w.outerHeight()||b.grid.prevRowHeight){w=D*z;z*=parseInt(b.p.records,10);a(">div:first",b.grid.bDiv).css({height:z}).children("div:first").css({height:w,
display:w?"":"none"})}b.grid.bDiv.scrollLeft=b.grid.hDiv.scrollLeft}P=b.p.pager?b.p.pager:"";P+=b.p.toppager?P?","+b.p.toppager:b.p.toppager:"";if(P){K=a.jgrid.formatter.integer||{};w=m(b.p.page);z=m(b.p.lastpage);a(".selbox",P).attr("disabled",false);if(b.p.pginput===true){a(".ui-pg-input",P).val(b.p.page);a("#sp_1"+I,P).html(a.fmatter?a.fmatter.util.NumberFormat(b.p.lastpage,K):b.p.lastpage)}if(b.p.viewrecords)if(b.p.reccount===0)a(".ui-paging-info",P).html(b.p.emptyrecords);else{R=D+1;V=b.p.records;
if(a.fmatter){R=a.fmatter.util.NumberFormat(R,K);H=a.fmatter.util.NumberFormat(H,K);V=a.fmatter.util.NumberFormat(V,K)}a(".ui-paging-info",P).html(a.jgrid.format(b.p.recordtext,R,H,V))}if(b.p.pgbuttons===true){if(w<=0)w=z=0;if(w==1||w===0){a("#first"+I+", #prev"+I).addClass("ui-state-disabled").removeClass("ui-state-hover");b.p.toppager&&a("#first_t"+I+", #prev_t"+I).addClass("ui-state-disabled").removeClass("ui-state-hover")}else{a("#first"+I+", #prev"+I).removeClass("ui-state-disabled");b.p.toppager&&
a("#first_t"+I+", #prev_t"+I).removeClass("ui-state-disabled")}if(w==z||w===0){a("#next"+I+", #last"+I).addClass("ui-state-disabled").removeClass("ui-state-hover");b.p.toppager&&a("#next_t"+I+", #last_t"+I).addClass("ui-state-disabled").removeClass("ui-state-hover")}else{a("#next"+I+", #last"+I).removeClass("ui-state-disabled");b.p.toppager&&a("#next_t"+I+", #last_t"+I).removeClass("ui-state-disabled")}}}u===true&&b.p.rownumbers===true&&a("td.jqgrid-rownum",b.rows).each(function(N){a(this).html(D+
1+N)});t&&b.p.jqgdnd&&a(b).jqGrid("gridDnD","updateDnD");a.isFunction(b.p.gridComplete)&&b.p.gridComplete.call(b)};b.refreshIndex=B;b.formatter=function(u,t,w,z,D){return r(u,t,w,z,D)};a.extend(g,{populate:ja,emptyRows:J});this.grid=g;b.addXmlData=function(u){O(u,b.grid.bDiv)};b.addJSONData=function(u){C(u,b.grid.bDiv)};this.grid.cols=this.rows[0].cells;ja();b.p.hiddengrid=false;a(window).unload(function(){b=null})}}}})};a.jgrid.extend({getGridParam:function(d){var e=this[0];if(e&&e.grid)return d?
typeof e.p[d]!="undefined"?e.p[d]:null:e.p},setGridParam:function(d){return this.each(function(){this.grid&&typeof d==="object"&&a.extend(true,this.p,d)})},getDataIDs:function(){var d=[],e=0,c,f=0;this.each(function(){if((c=this.rows.length)&&c>0)for(;e<c;){if(a(this.rows[e]).hasClass("jqgrow")){d[f]=this.rows[e].id;f++}e++}});return d},setSelection:function(d,e){return this.each(function(){function c(b){var p=a(f.grid.bDiv)[0].clientHeight,k=a(f.grid.bDiv)[0].scrollTop,n=f.rows[b].offsetTop;b=f.rows[b].clientHeight;
if(n+b>=p+k)a(f.grid.bDiv)[0].scrollTop=n-(p+k)+b+k;else if(n<p+k)if(n<k)a(f.grid.bDiv)[0].scrollTop=n}var f=this,g,h,j;if(d!==undefined){e=e===false?false:true;h=f.rows.namedItem(d+"");if(!(!h||h.className.indexOf("ui-state-disabled")>-1)){if(f.p.scrollrows===true){g=f.rows.namedItem(d).rowIndex;g>=0&&c(g)}if(f.p.multiselect){f.p.selrow=h.id;j=a.inArray(f.p.selrow,f.p.selarrrow);if(j===-1){h.className!=="ui-subgrid"&&a(h).addClass("ui-state-highlight").attr("aria-selected","true");g=true;a("#jqg_"+
a.jgrid.jqID(f.p.id)+"_"+a.jgrid.jqID(f.p.selrow)).attr("checked",g);f.p.selarrrow.push(f.p.selrow)}else{h.className!=="ui-subgrid"&&a(h).removeClass("ui-state-highlight").attr("aria-selected","false");g=false;a("#jqg_"+a.jgrid.jqID(f.p.id)+"_"+a.jgrid.jqID(f.p.selrow)).attr("checked",g);f.p.selarrrow.splice(j,1);j=f.p.selarrrow[0];f.p.selrow=j===undefined?null:j}f.p.onSelectRow&&e&&f.p.onSelectRow.call(f,h.id,g)}else if(h.className!=="ui-subgrid"){if(f.p.selrow!=h.id){a(f.rows.namedItem(f.p.selrow)).removeClass("ui-state-highlight").attr({"aria-selected":"false",
...
this.searchoptions.sopt?this.searchoptions.sopt[0]:"eq";if(p=a("#gs_"+a.jgrid.jqID(k),c.grid.hDiv).val()){j[k]=p;n[k]=l;b++}else try{delete c.p.postData[k]}catch(s){}break;case "text":l=this.searchoptions&&this.searchoptions.sopt?this.searchoptions.sopt[0]:d.defaultSearch;if(p=a("#gs_"+a.jgrid.jqID(k),c.grid.hDiv).val()){j[k]=p;n[k]=l;b++}else try{delete c.p.postData[k]}catch(x){}}});var o=b>0?true:false;if(d.stringResult===true||c.p.datatype=="local"){var m='{"groupOp":"'+d.groupOp+'","rules":[',
...
l){if(l>0)return n}).join(":");if(g){if(jQuery.inArray(j[0],h)>-1){f[p]=j[1];p++}}else if(a.trim(j[0])==a.trim(d)){f[0]=j[1];break}}else if(a.fmatter.isObject(c))if(g)f=jQuery.map(h,function(n){return c[n]});else f[0]=c[d]||""}d=f.join(", ");return d===""?a.fn.fmatter.defaultFormat(d,e):d};a.fn.fmatter.rowactions=function(d,e,c,f){var g={keys:false,editbutton:true,delbutton:true,onEdit:null,onSuccess:null,afterSave:null,onError:null,afterRestore:null,extraparam:{oper:"edit"},url:null,delOptions:{}};
f=a("#"+e)[0].p.colModel[f];a.fmatter.isUndefined(f.formatoptions)||(g=a.extend(g,f.formatoptions));f=function(j){g.afterSave&&g.afterSave(j);a("tr#"+d+" div.ui-inline-edit, tr#"+d+" div.ui-inline-del","#"+e).show();a("tr#"+d+" div.ui-inline-save, tr#"+d+" div.ui-inline-cancel","#"+e).hide()};var h=function(j){g.afterRestore&&g.afterRestore(j);a("tr#"+d+" div.ui-inline-edit, tr#"+d+" div.ui-inline-del","#"+e).show();a("tr#"+d+" div.ui-inline-save, tr#"+d+" div.ui-inline-cancel","#"+e).hide()};switch(c){case "edit":a("#"+
e).jqGrid("editRow",d,g.keys,g.onEdit,g.onSuccess,g.url,g.extraparam,f,g.onError,h);a("tr#"+d+" div.ui-inline-edit, tr#"+d+" div.ui-inline-del","#"+e).hide();a("tr#"+d+" div.ui-inline-save, tr#"+d+" div.ui-inline-cancel","#"+e).show();break;case "save":if(a("#"+e).jqGrid("saveRow",d,g.onSuccess,g.url,g.extraparam,f,g.onError,h)){a("tr#"+d+" div.ui-inline-edit, tr#"+d+" div.ui-inline-del","#"+e).show();a("tr#"+d+" div.ui-inline-save, tr#"+d+" div.ui-inline-cancel","#"+e).hide()}break;case "cancel":a("#"+
e).jqGrid("restoreRow",d,h);a("tr#"+d+" div.ui-inline-edit, tr#"+d+" div.ui-inline-del","#"+e).show();a("tr#"+d+" div.ui-inline-save, tr#"+d+" div.ui-inline-cancel","#"+e).hide();break;case "del":a("#"+e).jqGrid("delGridRow",d,g.delOptions)}};a.fn.fmatter.actions=function(d,e){var c={keys:false,editbutton:true,delbutton:true};a.fmatter.isUndefined(e.colModel.formatoptions)||(c=a.extend(c,e.colModel.formatoptions));var f=e.rowId,g="",h;if(typeof f=="undefined"||a.fmatter.isEmpty(f))return"";if(c.editbutton){h=
"onclick=$.fn.fmatter.rowactions('"+f+"','"+e.gid+"','edit',"+e.pos+");";g=g+"<div style='margin-left:8px;'><div title='"+a.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+h+"><span class='ui-icon ui-icon-pencil'></span></div>"}if(c.delbutton){h="onclick=$.fn.fmatter.rowactions('"+f+"','"+e.gid+"','del',"+e.pos+");";g=g+"<div title='"+a.jgrid.nav.deltitle+"' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' "+h+"><span class='ui-icon ui-icon-trash'></span></div>"}h=
"onclick=$.fn.fmatter.rowactions('"+f+"','"+e.gid+"','save',"+e.pos+");";g=g+"<div title='"+a.jgrid.edit.bSubmit+"' style='float:left;display:none' class='ui-pg-div ui-inline-save'><span class='ui-icon ui-icon-disk' "+h+"></span></div>";h="onclick=$.fn.fmatter.rowactions('"+f+"','"+e.gid+"','cancel',"+e.pos+");";return g=g+"<div title='"+a.jgrid.edit.bCancel+"' style='float:left;display:none;margin-left:5px;' class='ui-pg-div ui-inline-cancel'><span class='ui-icon ui-icon-cancel' "+h+"></span></div></div>"};
a.unformat=function(d,e,c,f){var g,h=e.colModel.formatter,j=e.colModel.formatoptions||{},b=/([\.\*\_\'\(\)\{\}\+\?\\])/g,p=e.colModel.unformat||a.fn.fmatter[h]&&a.fn.fmatter[h].unformat;if(typeof p!=="undefined"&&a.isFunction(p))g=p(a(d).text(),e,d);else if(!a.fmatter.isUndefined(h)&&a.fmatter.isString(h)){g=a.jgrid.formatter||{};switch(h){case "integer":j=a.extend({},g.integer,j);e=j.thousandsSeparator.replace(b,"\\$1");g=a(d).text().replace(RegExp(e,"g"),"");break;case "number":j=a.extend({},g.number,
j);e=j.thousandsSeparator.replace(b,"\\$1");g=a(d).text().replace(RegExp(e,"g"),"").replace(j.decimalSeparator,".");break;case "currency":j=a.extend({},g.currency,j);e=j.thousandsSeparator.replace(b,"\\$1");g=a(d).text().replace(RegExp(e,"g"),"").replace(j.decimalSeparator,".").replace(j.prefix,"").replace(j.suffix,"");break;case "checkbox":j=e.colModel.editoptions?e.colModel.editoptions.value.split(":"):["Yes","No"];g=a("input",d).attr("checked")?j[0]:j[1];break;case "select":g=a.unformat.select(d,
e,c,f);break;case "actions":return"";default:g=a(d).text()}}return g?g:f===true?a(d).text():a.jgrid.htmlDecode(a(d).html())};a.unformat.select=function(d,e,c,f){c=[];d=a(d).text();if(f===true)return d;e=a.extend({},e.colModel.editoptions);if(e.value){var g=e.value;e=e.multiple===true?true:false;f=[];var h;if(e){f=d.split(",");f=a.map(f,function(k){return a.trim(k)})}if(a.fmatter.isString(g))for(var j=g.split(";"),b=0,p=0;p<j.length;p++){h=j[p].split(":");if(h.length>2)h[1]=jQuery.map(h,function(k,
n){if(n>0)return k}).join(":");if(e){if(jQuery.inArray(h[1],f)>-1){c[b]=h[0];b++}}else if(a.trim(h[1])==a.trim(d)){c[0]=h[0];break}}else if(a.fmatter.isObject(g)||a.isArray(g)){e||(f[0]=d);c=jQuery.map(f,function(k){var n;a.each(g,function(l,o){if(o==k){n=l;return false}});if(typeof n!="undefined")return n})}return c.join(", ")}else return d||""};a.unformat.date=function(d,e){var c=a.jgrid.formatter.date||{};a.fmatter.isUndefined(e.formatoptions)||(c=a.extend({},c,e.formatoptions));return a.fmatter.isEmpty(d)?
...
o=a("<tr></tr>"),m,v,q,r,s="",x;o.append("<td class='first'></td>");var y=a("<td class='columns'></td>");o.append(y);var A=a("<select></select>"),F,J=[];y.append(A);A.bind("change",function(){k.field=a(A).val();q=a(this).parents("tr:first");for(m=0;m<l.p.columns.length;m++)if(l.p.columns[m].name===k.field){r=l.p.columns[m];break}if(r){r.searchoptions.id=a.jgrid.randId();if(b&&r.inputtype==="text")if(!r.searchoptions.size)r.searchoptions.size=10;var C=a.jgrid.createEl(r.inputtype,r.searchoptions,"",
true,l.p.ajaxSelectOptions,true);a(C).addClass("input-elm");v=r.searchoptions.sopt?r.searchoptions.sopt:l.p.sopt?l.p.sopt:r.searchtype==="string"?l.p.stropts:l.p.numopts;var S="",aa="";J=[];a.each(l.p.ops,function(){J.push(this.name)});for(m=v.length-1;m>=0;m--){F=a.inArray(v[m],J);if(F!==-1){k.op=l.p.ops[F].name;aa=m==0?" selected='selected'":"";S+="<option value='"+l.p.ops[F].name+"'"+aa+">"+l.p.ops[F].description+"</option>"}}a(".selectopts",q).empty().append(S);a(".data",q).empty().append(C);
...
"";C.value="";C.setAttribute("readonly","true");C.setAttribute("disabled","true")}else{C.removeAttribute("readonly");C.removeAttribute("disabled")}l.onchange()});v=r.searchoptions.sopt?r.searchoptions.sopt:l.p.sopt?l.p.sopt:r.searchtype==="string"?f.stropts:l.p.numopts;s="";a.each(l.p.ops,function(){J.push(this.name)});for(m=0;m<v.length;m++){F=a.inArray(v[m],J);if(F!==-1){x=k.op===l.p.ops[F].name?" selected='selected'":"";s+="<option value='"+l.p.ops[F].name+"'"+x+">"+l.p.ops[F].description+"</option>"}}O.append(s);
...
m="",v="",q,r=false,s=-1;if(e.showQuery)m="<a href='javascript:void(0)' id='"+h+"_query' class='fm-button ui-state-default ui-corner-all fm-button-icon-left'><span class='ui-icon ui-icon-comment'></span>Query</a>";if(e.columns.length)n=e.columns;else a.each(n,function(y,A){if(!A.label)A.label=g.p.colNames[y];if(!r){var F=typeof A.search==="undefined"?true:A.search,J=A.hidden===true;if(A.searchoptions&&A.searchoptions.searchhidden===true&&F||F&&!J){r=true;q=A.index||A.name;s=y}}});if(!p&&q||e.multipleSearch===
...
function(){a(".queryresult",k).toggle();return false});if(e.stringResult===undefined)e.stringResult=e.multipleSearch;a("#"+h+"_search").bind("click",function(){var y=a("#"+h),A={},F,J=y.jqFilter("filterData");if(e.errorcheck){y[0].hideError();e.showQuery||y.jqFilter("toSQLString");if(y[0].p.error){y[0].showError();return false}}if(e.stringResult){try{F=xmlJsonClass.toJson(J,"","",false)}catch(B){try{F=JSON.stringify(J)}catch(O){}}if(typeof F==="string"){A[e.sFilter]=F;a.each([e.sField,e.sValue,e.sOper],
function(){A[this]=""})}}else if(e.multipleSearch){A[e.sFilter]=J;a.each([e.sField,e.sValue,e.sOper],function(){A[this]=""})}else{A[e.sField]=J.rules[0].field;A[e.sValue]=J.rules[0].data;A[e.sOper]=J.rules[0].op;A[e.sFilter]=""}g.p.search=true;a.extend(g.p.postData,A);if(a.isFunction(e.onSearch))e.onSearch();a(g).trigger("reloadGrid",[{page:1}]);e.closeAfterSearch&&a.jgrid.hideModal("#"+b.themodal,{gb:"#gbox_"+g.p.id,jqm:e.jqModal,onClose:e.onClose});return false});a("#"+h+"_reset").bind("click",
...
this.editoptions.custom_value(a("#"+a.jgrid.jqID(G),"#"+r),"get");if(C[G]===undefined)throw"e1";}catch(L){L==="e1"?a.jgrid.info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.novalue,jQuery.jgrid.edit.bClose):a.jgrid.info_dialog(jQuery.jgrid.errors.errcap,L.message,jQuery.jgrid.edit.bClose)}return true}})}else{switch(a(this).get(0).type){case "checkbox":if(a(this).attr("checked"))C[this.name]=a(this).val();else{E=a(this).attr("offval");C[this.name]=E}break;case "select-one":C[this.name]=
a("option:selected",this).val();S[this.name]=a("option:selected",this).text();break;case "select-multiple":C[this.name]=a(this).val();C[this.name]=C[this.name]?C[this.name].join(","):"";var U=[];a("option:selected",this).each(function(L,Q){U[L]=a(Q).text()});S[this.name]=U.join(",");break;case "password":case "text":case "textarea":case "button":C[this.name]=a(this).val()}if(m.p.autoencode)C[this.name]=a.jgrid.htmlEncode(C[this.name])}});return true}function g(E,G,U,L){var Q,W,Y,ha=0,Z,ma,ea,ka=[],
ca=false,ta="",oa;for(oa=1;oa<=L;oa++)ta+="<td class='CaptionTD'>&#160;</td><td class='DataTD'>&#160;</td>";if(E!="_empty")ca=a(G).jqGrid("getInd",E);a(G.p.colModel).each(function(xa){Q=this.name;ma=(W=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false)?"style='display:none'":"";if(Q!=="cb"&&Q!=="subgrid"&&this.editable===true&&Q!=="rn"){if(ca===false)Z="";else if(Q==G.p.ExpandColumn&&G.p.treeGrid===true)Z=a("td:eq("+xa+")",G.rows[ca]).text();else try{Z=a.unformat(a("td:eq("+
...
t,Z,false,a.extend({},a.jgrid.ajaxOptions,G.p.ajaxSelectOptions||{}));if(Z===""&&this.edittype=="checkbox")Z=a(ea).attr("offval");if(Z===""&&this.edittype=="select")Z=a("option:eq(0)",ea).text();if(d.checkOnSubmit||d.checkOnUpdate)d._savedData[Q]=Z;a(ea).addClass("FormElement");if(this.edittype=="text"||this.edittype=="textarea")a(ea).addClass("ui-widget-content ui-corner-all");Y=a(U).find("tr[rowpos="+z+"]");if(w.rowabove){t=a("<tr><td class='contentinfo' colspan='"+L*2+"'>"+w.rowcontent+"</td></tr>");
a(U).append(t);t[0].rp=z}if(Y.length===0){Y=a("<tr "+ma+" rowpos='"+z+"'></tr>").addClass("FormData").attr("id","tr_"+Q);a(Y).append(ta);a(U).append(Y);Y[0].rp=z}a("td:eq("+(D-2)+")",Y[0]).html(typeof w.label==="undefined"?G.p.colNames[xa]:w.label);a("td:eq("+(D-1)+")",Y[0]).append(w.elmprefix).append(ea).append(w.elmsuffix);ka[ha]=xa;ha++}});if(ha>0){oa=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(L*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='"+
G.p.id+"_id' value='"+E+"'/></td></tr>");oa[0].rp=ha+999;a(U).append(oa);if(d.checkOnSubmit||d.checkOnUpdate)d._savedData[G.p.id+"_id"]=E}return ka}function h(E,G,U){var L,Q=0,W,Y,ha,Z,ma;if(d.checkOnSubmit||d.checkOnUpdate){d._savedData={};d._savedData[G.p.id+"_id"]=E}var ea=G.p.colModel;if(E=="_empty"){a(ea).each(function(){L=this.name;ha=a.extend({},this.editoptions||{});if((Y=a("#"+a.jgrid.jqID(L),"#"+U))&&Y.length&&Y[0]!==null){Z="";if(ha.defaultValue){Z=a.isFunction(ha.defaultValue)?ha.defaultValue():
ha.defaultValue;if(Y[0].type=="checkbox"){ma=Z.toLowerCase();if(ma.search(/(false|0|no|off|undefined)/i)<0&&ma!==""){Y[0].checked=true;Y[0].defaultChecked=true;Y[0].value=Z}else Y.attr({checked:"",defaultChecked:""})}else Y.val(Z)}else if(Y[0].type=="checkbox"){Y[0].checked=false;Y[0].defaultChecked=false;Z=a(Y).attr("offval")}else if(Y[0].type&&Y[0].type.substr(0,6)=="select")Y[0].selectedIndex=0;else Y.val(Z);if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[L]=Z}});a("#id_g","#"+U).val(E)}else{var ka=
a(G).jqGrid("getInd",E,true);if(ka){a("td",ka).each(function(ca){L=ea[ca].name;if(L!=="cb"&&L!=="subgrid"&&L!=="rn"&&ea[ca].editable===true){if(L==G.p.ExpandColumn&&G.p.treeGrid===true)W=a(this).text();else try{W=a.unformat(a(this),{rowId:E,colModel:ea[ca]},ca)}catch(ta){W=a(this).text()}if(m.p.autoencode)W=a.jgrid.htmlDecode(W);if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[L]=W;L=a.jgrid.jqID(L);switch(ea[ca].edittype){case "password":case "text":case "button":case "image":a("#"+L,"#"+
U).val(W);break;case "textarea":if(W=="&nbsp;"||W=="&#160;"||W.length==1&&W.charCodeAt(0)==160)W="";a("#"+L,"#"+U).val(W);break;case "select":var oa=W.split(",");oa=a.map(oa,function(u){return a.trim(u)});a("#"+L+" option","#"+U).each(function(){this.selected=!ea[ca].editoptions.multiple&&(oa[0]==a.trim(a(this).text())||oa[0]==a.trim(a(this).val()))?true:ea[ca].editoptions.multiple?a.inArray(a.trim(a(this).text()),oa)>-1||a.inArray(a.trim(a(this).val()),oa)>-1?true:false:false});break;case "checkbox":W+=
"";if(ea[ca].editoptions&&ea[ca].editoptions.value)if(ea[ca].editoptions.value.split(":")[0]==W){a("#"+L,"#"+U).attr("checked",true);a("#"+L,"#"+U).attr("defaultChecked",true)}else{a("#"+L,"#"+U).attr("checked",false);a("#"+L,"#"+U).attr("defaultChecked","")}else{W=W.toLowerCase();if(W.search(/(false|0|no|off|undefined)/i)<0&&W!==""){a("#"+L,"#"+U).attr("checked",true);a("#"+L,"#"+U).attr("defaultChecked",true)}else{a("#"+L,"#"+U).attr("checked",false);a("#"+L,"#"+U).attr("defaultChecked","")}}break;
case "custom":try{if(ea[ca].editoptions&&a.isFunction(ea[ca].editoptions.custom_value))ea[ca].editoptions.custom_value(a("#"+L,"#"+U),"set",W);else throw"e1";}catch(xa){xa=="e1"?a.jgrid.info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose):a.jgrid.info_dialog(jQuery.jgrid.errors.errcap,xa.message,jQuery.jgrid.edit.bClose)}}Q++}});Q>0&&a("#id_g","#"+r).val(E)}}}function j(){var E,G=[true,"",""],U={},L=m.p.prmNames,Q,W,Y,ha;if(a.isFunction(d.beforeCheckValues)){var Z=
d.beforeCheckValues(C,a("#"+q),C[m.p.id+"_id"]=="_empty"?L.addoper:L.editoper);if(Z&&typeof Z==="object")C=Z}for(Y in C)if(C.hasOwnProperty(Y)){G=a.jgrid.checkValues(C[Y],Y,m);if(G[0]===false)break}if(G[0]){if(a.isFunction(d.onclickSubmit))U=d.onclickSubmit(d,C)||{};if(a.isFunction(d.beforeSubmit))G=d.beforeSubmit(C,a("#"+q))}if(G[0]&&!d.processing){d.processing=true;a("#sData","#"+r+"_2").addClass("ui-state-active");W=L.oper;Q=L.id;C[W]=a.trim(C[m.p.id+"_id"])=="_empty"?L.addoper:L.editoper;if(C[W]!=
L.addoper)C[Q]=C[m.p.id+"_id"];else if(C[Q]===undefined)C[Q]=C[m.p.id+"_id"];delete C[m.p.id+"_id"];C=a.extend(C,d.editData,U);if(m.p.treeGrid===true){if(C[W]==L.addoper){ha=a(m).jqGrid("getGridParam","selrow");C[m.p.treeGridModel=="adjacency"?m.p.treeReader.parent_id_field:"parent_id"]=ha}for(i in m.p.treeReader){U=m.p.treeReader[i];if(C.hasOwnProperty(U))C[W]==L.addoper&&i==="parent_id_field"||delete C[U]}}U=a.extend({url:d.url?d.url:a(m).jqGrid("getGridParam","editurl"),type:d.mtype,data:a.isFunction(d.serializeEditData)?
...
a.jgrid.ajaxOptions,d.ajaxEditOptions);if(!U.url&&!d.useDataProxy)if(a.isFunction(m.p.dataProxy))d.useDataProxy=true;else{G[0]=false;G[1]+=" "+a.jgrid.errors.nourl}if(G[0])d.useDataProxy?m.p.dataProxy.call(m,U,"set_"+m.p.id):a.ajax(U)}if(G[0]===false){a("#FormError>td","#"+r).html(G[1]);a("#FormError","#"+r).show()}}function b(E,G){var U=false,L;for(L in E)if(E[L]!=G[L]){U=true;break}return U}function p(){a.each(m.p.colModel,function(E,G){if(G.editoptions&&G.editoptions.NullIfEmpty===true)if(C.hasOwnProperty(G.name)&&
...
d.beforeShowForm:false,y=a.isFunction(d.afterShowForm)?d.afterShowForm:false,A=a.isFunction(d.beforeInitData)?d.beforeInitData:false,F=a.isFunction(d.onInitializeForm)?d.onInitializeForm:false,J=true,B=1,O=0,C,S,aa,sa;if(e==="new"){e="_empty";c.caption=d.addCaption}else c.caption=d.editCaption;c.recreateForm===true&&a("#"+s.themodal).html()!==null&&a("#"+s.themodal).remove();var ja=true;if(c.checkOnUpdate&&c.jqModal&&!c.modal)ja=false;if(a("#"+s.themodal).html()!==null){if(A){J=A(a("#"+q));if(typeof J==
"undefined")J=true}if(J===false)return;n();a(".ui-jqdialog-title","#"+s.modalhead).html(c.caption);a("#FormError","#"+r).hide();if(d.topinfo){a(".topinfo","#"+r+"_2").html(d.topinfo);a(".tinfo","#"+r+"_2").show()}else a(".tinfo","#"+r+"_2").hide();if(d.bottominfo){a(".bottominfo","#"+r+"_2").html(d.bottominfo);a(".binfo","#"+r+"_2").show()}else a(".binfo","#"+r+"_2").hide();h(e,m,q);e=="_empty"||!d.viewPagerButtons?a("#pData, #nData","#"+r+"_2").hide():a("#pData, #nData","#"+r+"_2").show();if(d.processing===
...
y&&y(a("#"+q))}else{var ia=isNaN(c.dataheight)?c.dataheight:c.dataheight+"px";ia=a("<form name='FormPost' id='"+q+"' class='FormGrid' onSubmit='return false;' style='width:100%;overflow:auto;position:relative;height:"+ia+";'></form>").data("disabled",false);var wa=a("<table id='"+r+"' class='EditTable' cellspacing='0' cellpadding='0' border='0'><tbody></tbody></table>");if(A){J=A(a("#"+q));if(typeof J=="undefined")J=true}if(J===false)return;n();a(m.p.colModel).each(function(){var E=this.formoptions;
B=Math.max(B,E?E.colpos||0:0);O=Math.max(O,E?E.rowpos||0:0)});a(ia).append(wa);A=a("<tr id='FormError' style='display:none'><td class='ui-state-error' colspan='"+B*2+"'></td></tr>");A[0].rp=0;a(wa).append(A);A=a("<tr style='display:none' class='tinfo'><td class='topinfo' colspan='"+B*2+"'>"+d.topinfo+"</td></tr>");A[0].rp=0;a(wa).append(A);J=(A=m.p.direction=="rtl"?true:false)?"nData":"pData";var Aa=A?"pData":"nData";g(e,m,wa,B);J="<a href='javascript:void(0)' id='"+J+"' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>";
Aa="<a href='javascript:void(0)' id='"+Aa+"' class='fm-button ui-state-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>";var Ea="<a href='javascript:void(0)' id='sData' class='fm-button ui-state-default ui-corner-all'>"+c.bSubmit+"</a>",na="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default ui-corner-all'>"+c.bCancel+"</a>";J="<table border='0' cellspacing='0' cellpadding='0' class='EditTable' id='"+r+"_2'><tbody><tr><td colspan='2'><hr class='ui-widget-content' style='margin:1px'/></td></tr><tr id='Act_Buttons'><td class='navButton'>"+
(A?Aa+J:J+Aa)+"</td><td class='EditButton'>"+Ea+na+"</td></tr>";J+="<tr style='display:none' class='binfo'><td class='bottominfo' colspan='2'>"+d.bottominfo+"</td></tr>";J+="</tbody></table>";if(O>0){var Da=[];a.each(a(wa)[0].rows,function(E,G){Da[E]=G});Da.sort(function(E,G){if(E.rp>G.rp)return 1;if(E.rp<G.rp)return-1;return 0});a.each(Da,function(E,G){a("tbody",wa).append(G)})}c.gbox="#gbox_"+v;var va=false;if(c.closeOnEscape===true){c.closeOnEscape=false;va=true}ia=a("<span></span>").append(ia).append(J);
a.jgrid.createModal(s,ia,c,"#gview_"+m.p.id,a("#gbox_"+m.p.id)[0]);if(A){a("#pData, #nData","#"+r+"_2").css("float","right");a(".EditButton","#"+r+"_2").css("text-align","left")}d.topinfo&&a(".tinfo","#"+r+"_2").show();d.bottominfo&&a(".binfo","#"+r+"_2").show();J=ia=null;a("#"+s.themodal).keydown(function(E){var G=E.target;if(a("#"+q).data("disabled")===true)return false;if(d.savekey[0]===true&&E.which==d.savekey[1])if(G.tagName!="TEXTAREA"){a("#sData","#"+r+"_2").trigger("click");return false}if(E.which===
...
0,L,Q,W,Y=1;Y<=S;Y++)va+=Y==1?Da:"<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>";a(O.p.colModel).each(function(){sa=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false;if(!sa&&this.align==="right")if(this.formatter&&a.inArray(this.formatter,E)!==-1)G=Math.max(G,parseInt(this.width,10));else U=Math.max(U,parseInt(this.width,10))});L=G!==0?G:U!==0?U:0;na=a(O).jqGrid("getInd",B);a(O.p.colModel).each(function(ha){aa=
this.name;Q=false;Aa=(sa=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false)?"style='display:none'":"";W=typeof this.viewable!="boolean"?true:this.viewable;if(aa!=="cb"&&aa!=="subgrid"&&aa!=="rn"&&W){wa=na===false?"":aa==O.p.ExpandColumn&&O.p.treeGrid===true?a("td:eq("+ha+")",O.rows[na]).text():a("td:eq("+ha+")",O.rows[na]).html();Q=this.align==="right"&&L!==0?true:false;a.extend({},this.editoptions||{},{id:aa,name:aa});var Z=a.extend({},{rowabove:false,rowcontent:""},
...
"</b>");a("td:eq("+(ea-1)+")",ja[0]).append("<span>"+wa+"</span>").attr("id","v_"+aa);Q&&a("td:eq("+(ea-1)+") span",ja[0]).css({"text-align":"right",width:L+"px"});Ea[ia]=ha;ia++}});if(ia>0){B=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(S*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+B+"'/></td></tr>");B[0].rp=ia+99;a(C).append(B)}return Ea}function h(B,O){var C,S,aa=0,sa,ja;if(ja=a(O).jqGrid("getInd",B,true)){a("td",
...
d.afterSubmit(A,l);if(s[0]===false){a("#DelError>td","#"+n).html(s[1]);a("#DelError","#"+n).show()}else{if(d.reloadAfterSubmit&&f.p.datatype!="local")a(f).trigger("reloadGrid");else{var J=[];J=x.split(",");if(f.p.treeGrid===true)try{a(f).jqGrid("delTreeNode",J[0])}catch(B){}else for(var O=0;O<J.length;O++)a(f).jqGrid("delRowData",J[O]);f.p.selrow=null;f.p.selarrrow=[]}a.isFunction(d.afterComplete)&&setTimeout(function(){d.afterComplete(A,x)},500)}d.processing=false;a("#dData","#"+n+"_2").removeClass("ui-state-active");
...
!a(x).hasClass("not-editable-row")){y=o.p.colModel;a("td",x).each(function(A){m=y[A].name;var F=o.p.treeGrid===true&&m==o.p.ExpandColumn;if(F)v=a("span:first",this).html();else try{v=a.unformat(this,{rowId:d,colModel:y[A]},A)}catch(J){v=a(this).html()}if(m!="cb"&&m!="subgrid"&&m!="rn"){if(o.p.autoencode)v=a.jgrid.htmlDecode(v);s[m]=v;if(y[A].editable===true){if(r===null)r=A;F?a("span:first",this).html(""):a(this).html("");var B=a.extend({},y[A].editoptions||{},{id:d+"_"+m,name:m});if(!y[A].edittype)y[A].edittype=
...
d,true);if(x===false)return n;b=a(x).attr("editable");k.url=k.url?k.url:l.p.editurl;if(b==="1"){var y;a("td",x).each(function(J){y=l.p.colModel[J];o=y.name;if(o!="cb"&&o!="subgrid"&&y.editable===true&&o!="rn"&&!a(this).hasClass("not-editable-cell")){switch(y.edittype){case "checkbox":var B=["Yes","No"];if(y.editoptions)B=y.editoptions.value.split(":");m[o]=a("input",this).attr("checked")?B[0]:B[1];break;case "text":case "password":case "textarea":case "button":m[o]=a("input, textarea",this).val();
...
a.extend({},m,v);l.p.autoencode&&a.each(m,function(J,B){m[J]=a.jgrid.htmlDecode(B)});A=a(l).jqGrid("setRowData",d,m);a(x).attr("editable","0");for(b=0;b<l.p.savedRow.length;b++)if(l.p.savedRow[b].id==d){r=b;break}r>=0&&l.p.savedRow.splice(r,1);a.isFunction(k.aftersavefunc)&&k.aftersavefunc.call(l,d,A);n=true}else{a("#lui_"+l.p.id).show();q=a.extend({},m,q);a.ajax(a.extend({url:k.url,data:a.isFunction(l.p.serializeRowData)?l.p.serializeRowData.call(l,q):q,type:"POST",async:false,complete:function(J,
B){a("#lui_"+l.p.id).hide();if(B==="success")if((a.isFunction(k.succesfunc)?k.succesfunc.call(l,J):true)===true){l.p.autoencode&&a.each(m,function(C,S){m[C]=a.jgrid.htmlDecode(S)});m=a.extend({},m,v);a(l).jqGrid("setRowData",d,m);a(x).attr("editable","0");for(var O=0;O<l.p.savedRow.length;O++)if(l.p.savedRow[O].id==d){r=O;break}r>=0&&l.p.savedRow.splice(r,1);a.isFunction(k.aftersavefunc)&&k.aftersavefunc.call(l,d,J);n=true}else{a.isFunction(k.errorfunc)&&k.errorfunc.call(l,d,J,B);k.restoreAfterError===
true&&a(l).jqGrid("restoreRow",d,k.afterrestorefunc)}},error:function(J,B){a("#lui_"+l.p.id).hide();if(a.isFunction(k.errorfunc))k.errorfunc.call(l,d,J,B);else try{jQuery.jgrid.info_dialog(jQuery.jgrid.errors.errcap,'<div class="ui-state-error">'+J.responseText+"</div>",jQuery.jgrid.edit.bClose,{buttonalign:"right"})}catch(O){alert(J.responseText)}k.restoreAfterError===true&&a(l).jqGrid("restoreRow",d,k.afterrestorefunc)}},a.jgrid.ajaxOptions,l.p.ajaxRowOptions||{}))}a(x).unbind("keydown")}return n},
